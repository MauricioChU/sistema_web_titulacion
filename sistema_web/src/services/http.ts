...
export function getApiBaseUrl() {
  const maybeEnv = (import.meta as ImportMeta & { env?: { VITE_API_BASE_URL?: string } }).env;
...
function asErrorMessage(payload: unknown, fallback: string) {
  ...
  if (typeof payload === 'object') {
    ...
    if (Array.isArray(firstEntry) && typeof firstEntry[0] === 'string') return String(firstEntry[0]);
  }
