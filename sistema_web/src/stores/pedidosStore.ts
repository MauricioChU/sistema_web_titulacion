...
const CHECKLIST_TEMPLATE: ChecklistStep[] = [
  {
    id: 'materiales-listos',
    label: 'Materiales listos antes de salir',
    done: false,
  },
  ...
  {
    id: 'nota-adicional',
    label: 'Registre informacion adicional',
    done: false,
  },
];
...
function toPhaseFromApi(pedido: ApiPedido): PhaseKey {
  ...
  if (status === 'por-confirmar' || status === 'confirmado')
    return 'asignacion';
...
function mapApiUpdates(pedido: ApiPedido): TecnicoUpdate[] {
  return (pedido.tecnico_updates || []).map((entry) => ({
    ...
    note: entry.nota,
    status: entry.nuevo_estado
      ? statusLabel(entry.nuevo_estado as ApiPedido['status_operativo'])
      : undefined,
...
function mapApiChecklist(pedido: ApiPedido): ChecklistStep[] {
  ...
  return CHECKLIST_TEMPLATE.map((step) => {
    ...
    return {
      ...
      doneAt: source?.completado_en
        ? formatIsoAsStamp(source.completado_en)
        : undefined,
...
function mapApiReport(
  pedido: ApiPedido,
  informe: ApiInformeTecnico | null,
): ServiceReport[] {
  if (!informe) return [];
...
function mapApiPedido(
  pedido: ApiPedido,
  previous?: SharedPedido,
): SharedPedido {
  ...
  return {
    ...
    code:
      pedido.codigo ||
      previous?.code ||
      `A${String(pedido.id).slice(-4).padStart(4, '0')}`,
...
function toStateFromApi(pedidos: ApiPedido[]): BridgeState {
  ...
  pedidos.forEach((pedidoApi) => {
    ...
    const previous = coordinatorSnapshot.value.find(
...
const currentRole = ref<'admin' | 'coordinador' | 'tecnico' | 'usuario'>(
  'usuario',
);
...
async function refreshPedidosFromApi(force = false) {
  ...
  syncPromise = (async () => {
    ...
    const pedidos =
      me.role === 'tecnico' ? await listMisAsignados() : await listPedidos();
...
export function usePedidosStore() {
  ...
  async function refreshPedidoById(pedidoId: string) {
    ...
    const idx = coordinatorSnapshot.value.findIndex(
      (item) => item.id === pedidoId,
    );
    ...
    serviceReportsByPedido[pedidoId] = mapApiReport(
      apiPedido,
      apiPedido.informe_tecnico,
    );
  }
  ...
  function markChecklistStep(payload: {
    ...
    if (payload.done && !canMarkChecklistStep(payload.pedidoId, payload.stepId))
  ...
  function mergeWithCoordinatorPedidos(coordinatorPedidos: SharedPedido[]) {
    ...
    const coordinatorIds = new Set(
      coordinatorPedidos.map((pedido) => pedido.id),
    );
    const externos = tecnicoCreatedPedidos.value.filter(
      (pedido) => !coordinatorIds.has(pedido.id),
    );
  ...
  function addTecnicoUpdate(payload: {
    ...
    if (statusLower.includes('confirmado')) {
      return confirmarTecnico(payload.pedidoId).then(() =>
        refreshPedidoById(payload.pedidoId),
      );
    }
    ...
    updatesByPedido[payload.pedidoId] = [
      update,
      ...(updatesByPedido[payload.pedidoId] || []),
    ];
  ...
  function addEvidencias(payload: {
    ...
    const uploads = payload.items.map((item) => {
      const file = dataUrlToFile(
        item.url,
        item.name || `evidencia-${Date.now()}.jpg`,
      );
  ...
  function hasEvidenciasStage(pedidoId: string, stage: EvidenciaStage) {
    return (evidenciasByPedido[pedidoId] || []).some(
  ...
  function hasCompleteEvidenceSet(pedidoId: string) {
    return (
      hasEvidenciasStage(pedidoId, 'antes') &&
      hasEvidenciasStage(pedidoId, 'despues')
    );
  }
  ...
  function submitServiceReport(payload: {
    ...
    return submitInformeTecnico(payload.pedidoId, {
      ...
      const mapped = mapApiPedido(
        pedido,
        coordinatorSnapshot.value.find((item) => item.id === String(pedido.id)),
      );
  ...
  function updateDiagnostico(pedidoId: string, diagnostico: string) {
    return patchDiagnostico(pedidoId, diagnostico).then(() =>
      refreshPedidoById(pedidoId),
    );
  }
