...
type ModuleKey = 'inventario' | 'cuentas-clientes' | 'pedidos' | 'personal-campo' | 'reportes';
type GenericModuleKey = 'inventario' | 'pedidos' | 'personal-campo' | 'reportes';
...
const activeModuleSupportsRefresh = computed(
  () => activeModuleKey.value === 'pedidos' || activeModuleKey.value === 'inventario' || activeModuleKey.value === 'personal-campo'
);
...
const activeRefreshLabel = computed(() => {
  ...
  if (activeModuleKey.value === 'personal-campo') return 'Actualizar personal de campo';
  return 'Actualizar';
});
...
const activeGenericLoadError = computed(() => {
  ...
  if (activeModuleKey.value === 'personal-campo') return tecnicosLoadError.value;
  return '';
});
...
const filteredClients = computed(() => {
  ...
  return clientesRows.value.filter((cliente) => {
    ...
    return getClientAccounts(cliente.id).some((cuenta) =>
      [cuenta.codigo, cuenta.nombre, cuenta.direccion, cuenta.distrito, cuenta.contacto, cuenta.telefono, cuenta.estado]
        .join(' ')
        .toLowerCase()
        .includes(query)
    );
...
function parseEstadoActivo(estado: string | null | undefined, defaultValue: boolean) {
...
function parsePedidoStatus(
  ...
  const byCode = (Object.keys(pedidoStatusLabels) as Array<ApiPedido['status_operativo']>).find(
    (item) => item === normalized
  );
  if (byCode) return byCode;
  const byLabel = (Object.entries(pedidoStatusLabels) as Array<[ApiPedido['status_operativo'], string]>).find(
    ([, label]) => normalizePedidoToken(label) === normalized
  );
  return byLabel?.[0] || fallback;
}

function parsePedidoFase(value: string | null | undefined, fallback: ApiPedido['fase']): ApiPedido['fase'] {
  const normalized = normalizePedidoToken(value);
  if (!normalized) return fallback;
  const byCode = (Object.keys(pedidoFaseLabels) as Array<ApiPedido['fase']>).find((item) => item === normalized);
  if (byCode) return byCode;
  const byLabel = (Object.entries(pedidoFaseLabels) as Array<[ApiPedido['fase'], string]>).find(
    ([, label]) => normalizePedidoToken(label) === normalized
  );
  return byLabel?.[0] || fallback;
}
...
function parsePedidoPrioridad(
  ...
  const normalized = normalizePedidoToken(value);
  if (!normalized) return fallback;
  const byCode = (Object.keys(pedidoPrioridadLabels) as Array<ApiPedido['prioridad']>).find(
    (item) => item === normalized
  );
  if (byCode) return byCode;
  const byLabel = (Object.entries(pedidoPrioridadLabels) as Array<[ApiPedido['prioridad'], string]>).find(
    ([, label]) => normalizePedidoToken(label) === normalized
  );
  return byLabel?.[0] || fallback;
}
...
function mapPedidoToDataRow(pedido: ApiPedido): DataRow {
  return {
    ...
    servicio: normalizeValue(pedido.tipo_servicio || pedido.titulo, 'Sin servicio'),
    ...
    fecha: normalizeValue((pedido.fecha_programada || pedido.created_at || '').slice(0, 10), 'Sin fecha'),
  };
}
...
async function loadPedidosRows() {
  ...
  try {
    ...
    pedidosLoadError.value = toErrorMessage(error, 'No se pudo cargar pedidos desde backend.');
  } finally {
...
async function loadInventarioRows() {
  ...
  try {
    ...
    inventarioLoadError.value = toErrorMessage(error, 'No se pudo cargar inventario desde backend.');
  } finally {
...
async function loadTecnicosRows() {
  ...
  try {
    ...
    tecnicosLoadError.value = toErrorMessage(error, 'No se pudo cargar personal de campo desde backend.');
  } finally {
...
async function loadClientesCuentasRows() {
  ...
  try {
    ...
    clientesCuentasLoadError.value = toErrorMessage(error, 'No se pudo cargar clientes/cuentas desde backend.');
  } finally {
...
async function saveGenericRow() {
  ...
  if (moduleKey === 'pedidos') {
    ...
    if (!currentPedido) {
      pedidosLoadError.value = 'No se encontro el pedido en cache. Recarga la tabla.';
      showError(pedidosLoadError.value);
      return;
    }

    const nextStatus = parsePedidoStatus(formState.estado, currentPedido.status_operativo);
    if (
    ...
    const payload = {
      ...
      prioridad: parsePedidoPrioridad(formState.prioridad, currentPedido.prioridad),
    ...
    try {
      ...
      const message = toErrorMessage(error, 'No se pudo actualizar el pedido en backend.');
      pedidosLoadError.value = message;
      showError(message);
    }
    return;
  ...
  if (moduleKey === 'inventario') {
    ...
    try {
      ...
      await loadInventarioRows();
      cancelEdit();
      showSuccess(isCreate ? 'Item de inventario creado correctamente.' : 'Item de inventario actualizado correctamente.');
    } catch (error) {
      const message = toErrorMessage(error, 'No se pudo guardar inventario en backend.');
  ...
  if (moduleKey === 'personal-campo') {
    ...
    try {
      ...
      await loadTecnicosRows();
      cancelEdit();
      showSuccess(isCreate ? 'Tecnico creado correctamente.' : 'Tecnico actualizado correctamente.');
    } catch (error) {
      const message = toErrorMessage(error, 'No se pudo guardar personal de campo en backend.');
  ...
  if (editMode.value === 'create') {
    const newRow: DataRow = { id: `${Date.now()}-${Math.floor(Math.random() * 1000)}` };
  ...
  cancelEdit();
  showSuccess(editMode.value === 'create' ? 'Registro creado en vista local.' : 'Registro actualizado en vista local.');
}
...
function askRemoveGenericRow(row: DataRow) {
  ...
  const rowLabel = String((primaryColumn && row[primaryColumn]) || row.id || 'registro');
...
async function executeRemoveGenericRow(id: string, rowLabel: string) {
  ...
  if (moduleKey === 'inventario') {
    ...
    try {
      ...
      const message = toErrorMessage(error, 'No se pudo eliminar item de inventario.');
      inventarioLoadError.value = message;
      showError(message);
      throw error;
    }
  ...
  if (moduleKey === 'pedidos') {
    ...
    try {
      ...
      const message = toErrorMessage(error, 'No se pudo dar de baja el pedido.');
      pedidosLoadError.value = message;
      showError(message);
      throw error;
    }
  ...
  genericRows[moduleKey] = genericRows[moduleKey].filter((row) => row.id !== id);
...
async function executeRemoveClient(id: string, nombre: string) {
  ...
  try {
    ...
    const message = toErrorMessage(error, 'No se pudo eliminar cliente en backend.');
...
async function executeRemoveCuenta(clienteId: string, id: string, cuentaLabel: string) {
  clientesCuentasLoadError.value = '';
  try {
    ...
    const message = toErrorMessage(error, 'No se pudo eliminar cuenta en backend.');
    clientesCuentasLoadError.value = message;
    showError(message);
    throw error;
  }

  if (cuentaTargetClienteId.value === clienteId && cuentaEditingId.value === id) {
    goToClientesList();
  }
}
...
async function saveClienteForm() {
  ...
  try {
    ...
    await loadClientesCuentasRows();
    goToClientesList();
    showSuccess(isCreate ? 'Cliente creado correctamente.' : 'Cliente actualizado correctamente.');
  } catch (error) {
    const message = toErrorMessage(error, 'No se pudo guardar cliente en backend.');
    clientesCuentasLoadError.value = message;
    showError(message);
  }
}
...
async function saveCuentaForm() {
  ...
  if (latitud === null || longitud === null) {
    const message = 'Coordenadas invalidas. Usa latitud [-90, 90] y longitud [-180, 180].';
    clientesCuentasLoadError.value = message;
    showError(message);
    return;
  }
  ...
  try {
    ...
    await loadClientesCuentasRows();
    goToClientesList();
    showSuccess(isCreate ? 'Cuenta creada correctamente.' : 'Cuenta actualizada correctamente.');
  } catch (error) {
    const message = toErrorMessage(error, 'No se pudo guardar cuenta en backend.');
    clientesCuentasLoadError.value = message;
    showError(message);
  }
}
...
onMounted(() => {
  void Promise.all([loadPedidosRows(), loadClientesCuentasRows(), loadInventarioRows(), loadTecnicosRows()]);
});
